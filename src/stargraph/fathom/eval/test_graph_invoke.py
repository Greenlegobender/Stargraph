...
async def dummy_start(graph_id, config):
    ...
    print(f"📦 Configured targeting weights: {config['compiled_model_id']} (v{config['compiled_version']})")
...
async def main():
    ...
    result = await graph_invoke(
        graph_id="production_payment_clearance_flow", 
        initial_state_patch={"amount": 50.0}
    )
