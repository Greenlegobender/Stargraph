...
class CompileGateHarness:
    ...
    def _evaluate_single_scenario(self, args: Tuple[int, Any, Dict[str, Any]]) -> Tuple[int, int, Dict[str, int]]:
    ...
    def _call_cross_judge(self, scenario: Dict[str, Any], dialogue_log: List[Dict[str, Any]]) -> bool:
    ...
    def evaluate_gate(self, baseline: EvalResult, compiled: EvalResult) -> Tuple[bool, Dict[str, Any]]:
        """Compares baseline rules against compiled graphs using safe statistical gates."""
        p_val_success = self._safe_wilcoxon_p(baseline.raw_success_vector, compiled.raw_success_vector)
        ...
        for hub_id in baseline.raw_routing_failures:
            if hub_id in compiled.raw_routing_failures:
                hub_p_values[hub_id] = self._safe_wilcoxon_p(
                    baseline.raw_routing_failures[hub_id], 
                    compiled.raw_routing_failures[hub_id]
                )
        ...
        for rank_idx, (hub_id, raw_p) in enumerate(sorted_hubs):
            ...
            has_more_failures = compiled.routing_failures_per_hub.get(hub_id, 0) > baseline.routing_failures_per_hub.get(hub_id, 0)
    ...
    def _bootstrap_ci(self, data: np.ndarray, resamples: int = 1000) -> Tuple[float, float]:
        if len(data) == 0:
            return 0.0, 0.0
        boot_means = [np.mean(np.random.choice(data, size=len(data), replace=True)) for _ in range(resamples)]
